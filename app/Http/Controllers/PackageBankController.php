...
class PackageBankController extends Controller
{
    ...
    public function store(StorePackageBankRequest $request)
    {
         ...
         $newBank = DB::transaction(function() use ($request){
        ...
        return PackageBank::create($validated); // ⬅️ tambahkan return
        });

        return redirect()->route('admin.package_banks.index')->with('success', 'Bank berhasil ditambahkan.');;
        // logger('✅ MASUK STORE METHOD');
        // dd($request->all());
